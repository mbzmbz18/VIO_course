    ...
    :bStart_backend(true)
{
    ...
    readParameters(sConfig_file);//输出读取到了什么参数

    trackerData[0].readIntrinsicParameter(sConfig_file);

    estimator.setParameter();//?这个是要干啥???
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            for (unsigned int j = 0; j < ids.size(); j++)
            {
                if (trackerData[i].track_cnt[j] > 1)
                {
                    ...
                    feature_points->velocity_x_of_point.push_back(pts_velocity[j].x);//时间戳不对齐的问题
    ...
    const Eigen::Vector3d &vAcc)
{
    ...
    if (dStampSec < last_imu_t) //去掉等于的原因是因为第一帧imu数据的时间戳为0,会发出错误的信息
    {
    ...
    m_buf.lock();
    // 图像时间戳能够卡出一段Imu数据
    imu_buf.push(imu_msg);//将IMU数据放到队列里
    // cout << "1 PubImuData t: " << fixed << imu_msg->header 
    //     << " imu_buf size:" << imu_buf.size() << endl;
    m_buf.unlock();
    con.notify_one();//通知一个等待的线程
}
...
void System::ProcessBackEnd()
{
    ...
    while (bStart_backend)
    {
        ...
        con.wait(lk, [&] {
            return (measurements = getMeasurements()).size() != 0;//测量值,操作buffer
        });
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                if (t <= img_t)
                {
                    ...
                    estimator.processIMU(dt, Vector3d(dx, dy, dz), Vector3d(rx, ry, rz));//处理IMU数据,预处理IMU数据
            ...
            for (unsigned int i = 0; i < img_msg->points.size(); i++) 
            {
                ...
                image[feature_id].emplace_back(camera_id, xyz_uv_velocity);//遍历图像
            }
