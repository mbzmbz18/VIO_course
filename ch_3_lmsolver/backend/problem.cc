...
bool Problem::Solve(int iterations) 
{
    ...
    while (!stop && (iter < iterations)) {
        ...
        // 不断尝试阻尼系数Lambda, 直到成功迭代一步
        while (!oneStepSuccess) {  
        
            // Hessian对角线加上当前的阻尼系数Lambda
            AddLambdatoHessianLM();

            // 对当前的Lambda进行记录，记录到points.txt文件中
            ...
            // 优化退出条件1： delta_x_很小，则退出
            if (delta_x_.squaredNorm() <= 1e-6 || false_cnt > 10) {
                stop = true;
                break;
            }

            // 更新待估计状态量 X = X + delta_x
            UpdateStates();
...
void Problem::MakeHessian() 
{
    ...
    // 直接初始化大的H矩阵和b向量
    ulong size = ordering_generic_; // 得到待优化变量的局部参数化的总维度
    ...
    // 遍历问题中的每个残差，并计算他们的雅克比，得到最后的 H = J^T * J
...
// 解线性方程Hx = b, we can use PCG iterative method or use sparse Cholesky
...
// LM算法中用于判断阻尼引子Lambda在上次迭代中是否可以，以及Lambda怎么缩放
